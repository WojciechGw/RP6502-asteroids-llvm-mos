...
static uint16_t canvas_w_double;
static uint16_t canvas_h_double;
static uint16_t canvas_w_half;
static uint16_t canvas_h_half;
static uint16_t canvas_w_quarter;
static uint16_t canvas_h_quarter;
static uint16_t canvas_w_eight;
static uint16_t canvas_h_eight;
                          ...
                          uint8_t  bits_per_pixel)
{
    ...
    printf("canvas_mode: %i, bpp_mode: %i, canvas_struct: 0x%04X, plane: %i\n", 
            canvas_mode, bpp_mode, canvas_struct, plane);
    printf("canvas_w: %i, canvas_h: %i\n", canvas_w, canvas_h);

    // precalculate some common values
    canvas_w_double = canvas_w * 2;
    canvas_h_double = canvas_h * 2;
    canvas_w_half = canvas_w / 2;
    canvas_h_half = canvas_h / 2;
    canvas_w_quarter = canvas_w / 4;
    canvas_h_quarter = canvas_h / 4;
    canvas_w_eight = canvas_w / 8;
    canvas_h_eight = canvas_h / 8;
...
void erase_buffer(uint16_t buffer_data_address)
{
    ...
    if (bpp_mode == 4) { // 16bpp
        num_bytes = (canvas_w_double) * canvas_h;
    } else if (bpp_mode == 3) { // 8bpp
        num_bytes = canvas_w * canvas_h;
    } else if (bpp_mode == 2) { // 4bpp
        num_bytes = (canvas_w_half) * canvas_h;
    } else if (bpp_mode == 1) { //2bpp
        num_bytes = (canvas_w_quarter) * canvas_h;
    } else if (bpp_mode == 0) { //1bpp
        num_bytes = (canvas_w_eight) * canvas_h;
    }
...
void draw_pixel2buffer(uint16_t color, uint16_t x, uint16_t y, uint16_t buffer_data_address)
{
    ...
    switch (bpp_mode) {
        case 4: // 16bpp
            addr = buffer_data_address + (canvas_w_double * y + (x << 1)); // canvas_w * 2
            RIA.addr0 = addr;
            RIA.step0 = 1;
            RIA.rw0 = color;
            RIA.rw0 = color >> 8;
        ...
        case 3: // 8bpp
            addr = buffer_data_address + ((canvas_w * y) + x);
            RIA.addr0 = addr;
            RIA.step0 = 1;
            RIA.rw0 = color;
            break;
        ...
        case 2: // 4bpp
            shift = 4 * (1 - (x & 1)); // 4-bit shift for 4bpp
            addr = buffer_data_address + ((canvas_w_half * y) + (x >> 1)); // canvas_w / 2
            RIA.addr0 = addr;
            RIA.step0 = 0;
        ...
        case 1: // 2bpp
            shift = 2 * (3 - (x & 3)); // 2-bit shift for 2bpp
            addr = buffer_data_address + ((canvas_w_quarter * y) + (x >> 2)); // canvas_w / 4
            RIA.addr0 = addr;
            RIA.step0 = 0;
        ...
        case 0: // 1bpp
            shift = 7 - (x & 7); // 1-bit shift for 1bpp
            addr = buffer_data_address + ((canvas_w_eight * y) + (x >> 3)); // canvas_w / 8
            RIA.addr0 = addr;
            RIA.step0 = 0;
            RIA.rw0 = (RIA.rw0 & ~(1 << shift)) | ((color != 0) << shift);
            break;
    }
}
